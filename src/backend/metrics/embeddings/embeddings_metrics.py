...
def save_grafic(nome_arquivo, modelo, base_dir):
    modelo_dir = os.path.join(base_dir)

    # Criar diretório se não existir
    os.makedirs(modelo_dir, exist_ok=True)
    
    # Nome seguro para o arquivo (substituindo barras)
    safe_filename = nome_arquivo.replace("/", "_")
    
    # Caminho do arquivo
    caminho_completo = os.path.join(modelo_dir, f"{safe_filename}.png")
...
model_emb = HuggingFaceEmbeddings(model_name=embedding_model)

docsdb = FAISS.load_local("../../../../data/embeddings/all/", model_emb, allow_dangerous_deserialization=True) 
...
plt.hist(distancias, bins=50, alpha=0.75, color="blue")
plt.title("Distribuição das Distâncias Entre Embeddings")
plt.xlabel("Distância Euclidiana")
plt.ylabel("Frequência")
save_grafic("emb-" + embedding_model, embedding_model, "grafics/embeddings/distance")
plt.close()
...
query_embedding = model_emb.embed_query(query)
...
for doc in documentos:
    ...
    doc_embedding = model_emb.embed_query(doc["chunk"])
...
save_grafic("clust-" + embedding_model, embedding_model, "grafics/embeddings/clusters")
plt.close()